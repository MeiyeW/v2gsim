...
class CentralOptimization(object):
    ...
    def process(self, times, vehicles, d, pmax, pmin, emin, emax, efinal, 
        ...
        with SolverFactory(solver) as opt:
            ...
            # ###### Variable           
            model.u1= Var(model.t, model.v,  within=NonNegativeReals,doc='Power used for energy discharging')
            model.u2= Var(model.t, model.v,  within=NonNegativeReals,doc='Power used for energy charging')
            model.ub= Var(model.t, model.v,  within=Binary,doc='Power used for energy discharging')
            model.ub2= Var(model.t, model.v, within=Binary,doc='Power used for energy charging')
            ...
            def binary_rule(model, t, v):
                return model.ub[t,v] +model.ub2[t,v]+model.cb[t,v] +model.cb2[t,v]<= 1
            ...
            def maximum_power_rule4(model, t, v):
                return model.u2[t, v] <= model.p_max[t, v]
            model.power_max_rule4 = Constraint(model.t, model.v, rule=maximum_power_rule4, doc='P max rule')

            def minimum_power_rule4(model, t, v):
                return model.u2[t, v] >= model.p_min[t, v]
            model.power_min_rule4 = Constraint(model.t, model.v, rule=minimum_power_rule4, doc='P min rule')
            ...
            def minimum_energy_rule(model, t, v):
                return sum(model.u1[i, v]*model.ub[i,v] -model.u2[i, v]*model.ub2[i,v] + model.c1[i,v]*model.cb[i,v] - model.c2[i,v]*model.cb2[i,v] for i in range(0, t + 1)) >= model.e_min[t, v]
            ...
            def maximum_energy_rule(model, t, v):
                return sum(model.u1[i, v]*model.ub[i,v] -model.u2[i, v]*model.ub2[i,v] + model.c1[i,v]*model.cb[i,v] - model.c2[i,v]*model.cb2[i,v] for i in range(0, t + 1)) <= model.e_max[t, v]
            ...
            def final_energy_balance(model, v):
                return sum(model.u1[i, v]*model.ub[i,v] -model.u2[i, v]*model.ub2[i,v] + model.c1[i,v]*model.cb[i,v] - model.c2[i,v]*model.cb2[i,v] for i in model.t) >= model.e_final[v]
            ...
            if peak_shaving == 'economic':
                def objective_rule(model):
                    ...
                    -sum([model.u2[t, v]*model.ub2[t,v]*model.pr_e[t]*model.time_index for v in model.v])                    
                    ...
                    -sum([model.u2[t, v]*model.ub2[t,v]*0.1*model.time_index for v in model.v])                    
    ...
    def plot_result(self, model):
        ...
        # Get the result
        df = pandas.DataFrame(index=['power discharged'], data=model.u1.get_values()).transpose().groupby(level=0).sum()
        ...
        powerresult = pandas.concat([powerresult, df], axis=1)
        df = pandas.DataFrame(index=['power charged'], data=model.u2.get_values()).transpose().groupby(level=0).sum()
        powerresult = pandas.concat([powerresult, df], axis=1)
        df = pandas.DataFrame(index=['power charged binary'], data=model.ub2.get_values()).transpose().groupby(level=0).sum()
        powerresult = pandas.concat([powerresult, df], axis=1)
        ...
        for i in range(len(df)):
            discharge=powerresult[i]['power discharged']*powerresult[i]['power discharged binary']
            charge=powerresult[i]['power charged']*powerresult[i]['power charged binary']
            regup=powerresult[i]['power regulation up']*powerresult[i]['power regulation up binary']            
            regdown=powerresult[i]['power regulation down']*powerresult[i]['power regulation down binary']
            powersum=discharge[i]+charge[i]+regup[i]+regdown[i]
            powerresult.append((discharge[i],charge,regup,regdown,powersum))
