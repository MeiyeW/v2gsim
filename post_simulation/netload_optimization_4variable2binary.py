...
class CentralOptimization(object):
    ...
    def process(self, times, vehicles, d, pmax, pmin, emin, emax, efinal, 
        ...
        with SolverFactory(solver) as opt:
            ...
            # ###### Variable           
            model.u1= Var(model.t, model.v,  within=NonNegativeReals, doc='Power used by vehicle, charge')
            model.u2= Var(model.t, model.v,  within=NonNegativeReals, doc='Power used by vehicle, discharge')
            model.b1= Var(model.t, model.v,  within=Binary,doc='Power used for energy discharging')
            ...
            model.b2= Var(model.t, model.v,  within=Binary,doc='Power used for energy charging')


            # ###### Rules
            
            def binary_rule(model, t, v):
                return model.b1[t,v] +model.b2[t,v]<= 1
            model.binary_rule = Constraint(model.t, model.v, rule=binary_rule, doc='Binary rule')
            
            def maximum_power_rule1(model, t, v):
                return (model.u1[t, v]+model.c1[t,v])*model.b1[t,v]-(model.u2[t, v]+model.c2[t,v])*model.b2[t,v] <= model.p_max[t, v]
            ...
            def minimum_power_rule1(model, t, v):
                return (model.u1[t, v]+model.c1[t,v])*model.b1[t,v]-(model.u2[t, v]+model.c2[t,v])*model.b2[t,v] >= model.p_min[t, v]
            ...
            def minimum_energy_rule(model, t, v):
                return sum((model.u1[i, v]+ model.c1[i,v])*model.b1[i,v] -(model.u2[i,v]+model.c2[i,v]) *model.b2[i,v] for i in range(0, t + 1)) >= model.e_min[t, v]
            ...
            def maximum_energy_rule(model, t, v):
                return sum((model.u1[i, v]+ model.c1[i,v])*model.b1[i,v] -(model.u2[i,v]+model.c2[i,v]) *model.b2[i,v] for i in range(0, t + 1)) <= model.e_max[t, v]
            ...
            def final_energy_balance(model, v):
                return sum((model.u1[i, v]+ model.c1[i,v])*model.b1[i,v] -(model.u2[i,v]+model.c2[i,v]) *model.b2[i,v] for i in model.t) >= model.e_final[v]
            ...
            if peak_shaving == 'economic':
                def objective_rule(model):
                    return sum( [
                    -sum([model.u1[t, v]*model.b1[t,v]*model.pr_e[t]*model.time_index for v in model.v])
                    +sum([model.u2[t, v]*model.b2[t,v]*model.pr_e[t]*model.time_index for v in model.v])
                    +sum([model.c1[t, v]*model.b1[t,v]*model.pr_fre_c1[t]*model.time_index for v in model.v])
                    +sum([model.c2[t, v]*model.b2[t,v]*model.pr_fre_c2[t]*model.time_index for v in model.v])
                    +sum([model.c1[t, v]*model.b1[t,v]*model.re_energy_u*model.pr_fre_c1[t]*model.time_index for v in model.v])
                    +sum([model.c2[t, v]*model.b2[t,v]*model.re_energy_d*model.pr_fre_c2[t]*model.time_index for v in model.v])
                    -sum([model.u1[t, v]*model.b1[t,v]*0.1*model.time_index for v in model.v])   
                    -sum([model.u2[t, v]*model.b2[t,v]*0.1*model.time_index for v in model.v])   
                    -sum([model.c1[t, v]*model.b1[t,v]*0.1*model.time_index for v in model.v])
                    -sum([model.c2[t, v]*model.b2[t,v]*0.1*model.time_index for v in model.v])
                    for t in model.t] )
                    #model.pr_ba
