...
class CentralOptimization(object):
    ...
    def process(self, times, vehicles, d, pmax, pmin, emin, emax, efinal, 
        ...
        with SolverFactory(solver) as opt:
            ...
            # ###### Variable
            model.u = Var(model.t, model.v,  doc='Power used')
            model.u1= Var(model.t, model.v,  doc='Power used for energy discharging/charge')           
            ...
            def power_rule(model, t, v):
                ...
                #  + model.c1[t,v]*model.re_energy_u - model.c2[t,v]*model.re_energy_d
            ...
            # def capacity_binary_rule(model,t,v):
            #     return model.c1[t,v]*model.c2[t,v]<=0
            # model.capacity_binary_rule=Constraint(model.t, model.v, rule=capacity_binary_rule, doc='C Binary rule')
            ...
            if peak_shaving == 'economic':
                def objective_rule(model):                    
                    return sum( [
                    +sum([model.u1[t, v]*model.pr_e[t]*model.time_index for v in model.v])
                    ...
                    # -sum([model.u1[t, v]*0.1*model.time_index for v in model.v])
                    # -sum([model.u2[t, v]*0.1*model.time_index for v in model.v])
                    # -sum([model.c1[t, v]*0.1*model.time_index for v in model.v])
                    # -sum([model.c2[t, v]*0.1*model.time_index for v in model.v])
                    for t in model.t] )
                    #model.pr_ba
                model.objective = Objective(rule=objective_rule, sense=maximize, doc='Define objective function')

            elif peak_shaving == 'simple_economic':
                def objective_rule(model):                    
                    return sum( [
                    sum([model.u[t, v]*model.pr_e[t]*model.time_index for v in model.v])                    
                    for t in model.t] )
                    #model.pr_ba
                model.objective = Objective(rule=objective_rule, sense=maximize, doc='Define objective function')
